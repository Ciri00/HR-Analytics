...
(select count(attrition) from hrdata where attrition= 'Yes')) * 100, 2) as pct from 
hrdata 
where attrition='Yes' 
group by department  
order by count(attrition) desc; 
...
Job Satisfaction Rating -Run this query first to activate the cosstab() function in postgres 
...
FROM crosstab( 
 ...
 ) AS ct(job_role varchar(50), one numeric, two numeric, three numeric, 
four numeric) 
ORDER BY job_role;